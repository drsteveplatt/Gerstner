...
template<int gridCols, int gridRows> class Grid {
  ...
  public:
    ...
    int crToPixId(int col, int row) {
       if(col<0 || col>=m_gridCols*m_colsPerPane || row<0 || row>=m_gridRows*m_rowsPerPane) return 0;
...
template<> class Grid<1,1> {
  ...
  public:
    Grid(int nRows, int nCols, CRGB* leds): m_nRows(nRows), m_nCols(nCols) { 
      m_cells = leds;
      //m_wcsLLx = m_wcsLLy = 0;
